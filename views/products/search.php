...
<div class="container">
    <span><?= Yii::t('app', 'Show') ?>: <a class="nav-item dropdown-toggle bg-white text-dark" href="#" id="navbarDropdown1" role="button" data-bs-toggle="dropdown" aria-expanded="false">
        ...
        <ul class="dropdown-menu bg-white text-dark border" aria-labelledby="navbarDropdown1">
            <li><a class="dropdown-item text-dark" href="<?= Url::to(['products/search', 'searchData' => $_GET['searchData'], 'sort'=>'asc']); ?>"><?= Yii::t('app', 'Low-to-High') ?></a></li>
            <li><a class="dropdown-item text-dark" href="<?= Url::to(['products/search', 'searchData' => $_GET['searchData'], 'sort'=>'desc']); ?>"><?= Yii::t('app', 'High-to-Low') ?></a></li>
            <li><a class="dropdown-item text-dark" href="<?= Url::to(['products/search', 'searchData' => $_GET['searchData'], 'sort'=>'orders']); ?>"><?= Yii::t('app', 'By orders') ?></a></li>
            <li><a class="dropdown-item text-dark" href="<?= Url::to(['products/search', 'searchData' => $_GET['searchData'], 'sort'=> 'popularity']); ?>"><?= Yii::t('app', 'By popularity') ?></a></li>
            <li><a class="dropdown-item text-dark" href="<?= Url::to(['products/search', 'searchData' => $_GET['searchData'], 'sort'=> 'novelty']); ?>"><?= Yii::t('app', 'By novelty') ?></a></li>
        </ul>
    </span>
</div>
    ...
    <div class="col">
        <div class="row row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 py-2 mt-5 mx-2 bg-white text-dark">
            <?php if($data != null){ ?>
                ...
                <?php } ?>
            <?php } else { ?>
                <div class="alert alert-info"><?= Yii::t('app', 'No products found!') ?></div>
            <?php } ?>
        </div>
    </div>
</div>